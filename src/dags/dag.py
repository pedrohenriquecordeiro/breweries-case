...
# Definição do manifest diretamente no código
...
def pipeline_dag():
    ...
    bronze_task = KubernetesPodOperator(
        task_id                = "bronze_task",
        namespace              = "airflow",
        image                  = "us-central1-docker.pkg.dev/bees-463419/bees-docker-repo/bronze/bees-etl-bronze-job:latest",
    ...
    silver_task = SparkKubernetesOperator(
        ...
        application_file        = SPARK_APPLICATION_MANIFEST.replace("{IMAGE_SPARK}", "us-central1-docker.pkg.dev/bees-463419/bees-docker-repo/silver/bees-etl-silver-job:latest"),
    ...
    test_data_quality_test = SparkKubernetesOperator(
        ...
        startup_timeout_seconds = 600,
        delete_on_termination   = True,
        application_file        = SPARK_APPLICATION_MANIFEST.replace("{IMAGE_SPARK}", "us-central1-docker.pkg.dev/bees-463419/bees-docker-repo/tests/bees-test-data-quality:latest"),
        kubernetes_conn_id      = "in_cluster_configuration_kubernetes_cluster"
    )
    ...
    gold_task = SparkKubernetesOperator(
        ...
        startup_timeout_seconds = 600,
        delete_on_termination   = True,
        application_file        = SPARK_APPLICATION_MANIFEST.replace("{IMAGE_SPARK}", "us-central1-docker.pkg.dev/bees-463419/bees-docker-repo/gold/bees-etl-gold-job:latest"),
        kubernetes_conn_id      = "in_cluster_configuration_kubernetes_cluster"
    )
